...
import glob
import click
from molecules.data.dataset import traj_to_dset
...
@click.option("-e", "--pattern", default="*.dcd", help="Trajectory file pattern.")
@click.option(
    "-o", "--out_path", required=True, help="Path to file to write dataset to."
)
@click.option(
    "-w",
    "--num_workers",
...
@click.option(
    ...
    help="Atom selection for creating contact maps, " "point clouds and computing fnc.",
)
@click.option("--rmsd", is_flag=True, help="Computes and saves RMSD.")
@click.option(
    "--fnc", is_flag=True, help="Computes and saves fraction of reference contacts."
)
@click.option("--contact_map", is_flag=True, help="Computes and saves contact maps.")
@click.option(
    "-cmp",
...
@click.option("--point_cloud", is_flag=True, help="Computes and saves point cloud.")
@click.option(
    "-f",
    "--cm_format",
    default="sparse-concat",
...
@click.option("--distributed", is_flag=True, help="Uses MPI for distributed assembly.")
...
def main(
    ...
    if os.path.isdir(traj_path):
        ...
        if not traj_path:
            raise ValueError(
                "No traj files found in directory matching " f"the pattern {pattern}"
            )
    ...
    traj_to_dset(
        ...
        distance_kernel_params=contact_maps_parameters,
        sel=selection,
        cm_format=cm_format,
